// src/components/tools/modals/MoveToolStockModal.tsx
"use client";
...
export default function MoveToolStockModal({
	isOpen,
	onClose,
	tool,
	onDone,
	...
	async function submit() {
		try {
			setLoading(true);
			setErr(null);
			await ToolsService.moveStock(tool.id, { type, qty: Number(qty), note });
			onDone?.();
			onClose();
		} catch (e: any) {
	...
	return (
		<Modal isOpen={isOpen} onClose={onClose} className="max-w-md p-6 sm:p-8">
			<h3 className="mb-2 text-lg font-semibold">Inventario: {tool.name}</h3>
			<p className="mb-4 text-sm text-slate-600">
				Stock actual: <b>{tool.stock}</b>
			</p>
			{err && (
