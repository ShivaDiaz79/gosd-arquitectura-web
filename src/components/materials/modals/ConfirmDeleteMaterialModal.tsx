...
export default function ConfirmDeleteMaterialModal({
	isOpen,
	onClose,
	materialId,
	materialDesc,
	...
	async function onConfirm() {
		...
		try {
			...
			setError(e?.message ?? "No se pudo eliminar el material");
		} finally {
			setLoading(false);
		}
	}
	...
	return (
		<Modal isOpen={isOpen} onClose={onClose} className="max-w-md p-6 sm:p-8">
			<h3 className="mb-2 text-lg font-semibold">Eliminar material</h3>
			<p className="mb-5 text-sm text-gray-500">
				Esta acción es permanente para{" "}
				<span className="font-semibold">{materialDesc || materialId}</span>.
			</p>
			{done ? (
