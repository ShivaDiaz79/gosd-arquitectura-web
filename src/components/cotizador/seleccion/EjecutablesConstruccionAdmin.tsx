...
export default function EjecutablesConstruccionAdmin() {
	...
	const G = cfg?.EJECUTABLES_CONSTRUCCION_L1 || [];
	const S = cfg?.EJECUTABLES_CONSTRUCCION_L2 || {};
	...
	const addGrupo = async (d: FGrupo) => {
		...
		try {
			...
			const next = await CotizadorService.addEjecutableGrupo({ label, value });
			setCfg((p) => (p ? { ...p, EJECUTABLES_CONSTRUCCION_L1: next } : p));
			fG.reset({ g_label: "", g_value: "" });
		} catch (e: any) {
			setError(e?.message || "No se pudo guardar grupo.");
		} finally {
			setSaving(false);
		}
	};
	const rmGrupo = async (v: string) => {
		if (!confirm("Eliminar grupo y sus subcategorías?")) return;
		setSaving(true);
		try {
			await CotizadorService.removeEjecutableGrupo(v);
			setCfg(await CotizadorService.getConfig());
		} catch (e: any) {
			setError(e?.message || "No se pudo eliminar grupo.");
		} finally {
			setSaving(false);
		}
	};
	...
	const addSub = async (d: FSub) => {
		...
		try {
			...
			if (label.length < 2 || !/^[a-z0-9_]+$/.test(value))
				throw new Error("Datos inválidos");
			const next = await CotizadorService.addEjecutableSub(parent, {
				label,
				value,
			});
			setCfg((p) => (p ? { ...p, EJECUTABLES_CONSTRUCCION_L2: next } : p));
			fS.reset({ parent_g: parent, s_label: "", s_value: "" });
		} catch (e: any) {
			setError(e?.message || "No se pudo guardar subcategoría.");
		} finally {
			setSaving(false);
		}
	};
	const rmSub = async (parent: string, v: string) => {
		...
		setSaving(true);
		try {
			await CotizadorService.removeEjecutableSub(parent, v);
			setCfg(await CotizadorService.getConfig());
		} catch (e: any) {
			setError(e?.message || "No se pudo eliminar subcategoría.");
		} finally {
			setSaving(false);
		}
	};
	...
	const fR = useForm<FRangos>({
		defaultValues: { sub: "", rangos: [], min: "", max: "", tarifa_m2: "" },
		mode: "onTouched",
	});
	const subOpts = useMemo(() => {
		const arr: { value: string; label: string }[] = [];
		for (const g of G)
			for (const s of S[g.value] || [])
				arr.push({ value: s.value, label: `${g.label} → ${s.label}` });
		return arr;
	}, [G, S]);
	const subSel = fR.watch("sub");
	const rangosActuales = useMemo(
		() => (subSel ? RAN[subSel] || [] : []),
		[RAN, subSel]
	);
	useEffect(() => {
	...
	const addR = () => {
		const min = parseFloat(fR.getValues("min") || "");
		const maxStr = fR.getValues("max") || "";
		const max = maxStr === "" ? undefined : parseFloat(maxStr);
		const t = parseFloat(fR.getValues("tarifa_m2") || "");
		if (
			!subSel ||
			!Number.isFinite(min) ||
			(!Number.isNaN(max!) && max! <= min) ||
			!Number.isFinite(t)
		) {
			alert("Datos inválidos");
			return;
		}
		const curr = fR.getValues("rangos") || [];
		...
		fR.setValue("rangos", next);
		fR.setValue("min", "");
		fR.setValue("max", "");
		fR.setValue("tarifa_m2", "");
	};

	const saveR = async (d: FRangos) => {
		if (!subSel) {
			alert("Seleccione subcategoría");
			return;
		}
		setSaving(true);
		try {
	...
	return (
		<div className="space-y-10">
			...
			<section className="space-y-4 rounded-lg border p-4">
				<h2 className="text-lg font-semibold">Subcategorías (L2)</h2>
				<FormProvider {...fS}>
					<form
				...
				<div className="rounded-lg border">
					<div className="grid grid-cols-12 px-4 py-2 text-xs font-medium text-gray-500">
						...
						<div className="col-span-5">Subcategoría</div>
			...
			<section className="space-y-4 rounded-lg border p-4">
				<h2 className="text-lg font-semibold">
					Tarifas por m² (por subcategoría)
				</h2>
				<FormProvider {...fT}>
					<form
			...
			<section className="space-y-4 rounded-lg border p-4">
				<h2 className="text-lg font-semibold">Rangos por superficie (m²)</h2>
				<FormProvider {...fR}>
					<form onSubmit={fR.handleSubmit(saveR)} className="space-y-4">
						...
						<div className="rounded-lg border">
							...
							<div className="divide-y">
								{(fR.watch("rangos") || []).length === 0 && (
									<div className="px-4 py-4 text-sm text-gray-500">
										Sin rangos.
									</div>
								)}
								{(fR.watch("rangos") || []).map((r, i) => (
									<div
										key={i}
										className="grid grid-cols-12 px-4 py-3 items-center"
									>
						...
						<Button type="submit" disabled={saving || !subSel}>
			...
			<section className="space-y-4 rounded-lg border p-4">
				<h2 className="text-lg font-semibold">
					Matriz por categoría (elige SUBs)
				</h2>
				<FormProvider {...fM}>
