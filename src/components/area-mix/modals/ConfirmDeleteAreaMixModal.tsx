...
export default function ConfirmDeleteAreaMixModal({
	isOpen,
	onClose,
	mixId,
	mixName,
	...
	async function onConfirm() {
		...
		try {
			setLoading(true);
			setError(null);
			await AreaMixService.remove(mixId);
			setDone(true);
			onDeleted?.();
		} catch (e: any) {
	...
	return (
		<Modal isOpen={isOpen} onClose={onClose} className="max-w-md p-6 sm:p-8">
			<h3 className="mb-2 text-lg font-semibold">Eliminar perfil</h3>
			<p className="mb-5 text-sm text-gray-500">
				Esto es permanente para{" "}
				<span className="font-semibold">{mixName || mixId}</span>.
			</p>
